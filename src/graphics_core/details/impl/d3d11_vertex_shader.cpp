...
namespace alya::graphics::core::details
{
	...
	d3d11_vertex_shader::d3d11_vertex_shader(const void* bytecode, size_t size, d3d11_context&context)
		: bytecode_(static_cast<const char*>(bytecode), static_cast<const char*>(bytecode) + size)
	{
		ALYA_GFX_CALL(context.device().native_handle()->CreateVertexShader(bytecode, size, nullptr, &impl_));
